...
import { vueRoadmapSteps } from '../../data/vue-roadmap'
import { useVueRoadmapPost, useVueRoadmaps } from '../../composables/useVueRoadmaps'
...
const post = await useVueRoadmapPost(slug.value)

if (!post) {
  throw createError({ statusCode: 404, statusMessage: 'Vue.js roadmap article not found' })
}

const allPosts = await useVueRoadmaps()
const currentIndex = computed(() => allPosts.findIndex((item) => item.path === post.path))
...
const totalSteps = computed(() => allPosts.length || vueRoadmapSteps.length)
const articlePath = computed(() => localePath(post.path))
const canonicalUrl = computed(() => `${config.public.siteUrl}${articlePath.value}`)

const ui = computed(() =>
  locale.value === 'fr'
    ? {
        back: 'Retour à la roadmap Vue.js',
        previous: 'Étape précédente',
        next: 'Étape suivante',
        toc: 'Table des matières',
        noHeadings: 'Aucun titre dans cette page.',
        navAria: 'Navigation des pages de la roadmap Vue.js',
        progress: `Étape ${stepNumber.value} / ${totalSteps.value}`
      }
    : {
        back: 'Back to Vue.js Roadmap',
        previous: 'Previous step',
        next: 'Next step',
        toc: 'Table of contents',
        noHeadings: 'No headings in this page.',
        navAria: 'Vue.js roadmap article navigation',
        progress: `Step ${stepNumber.value} of ${totalSteps.value}`
      }
)
...
useSeoMeta({
  ...
  ogTitle: `${post.title} | Vue.js Roadmap`,
...
<template>
  <article class="blog-article-main space-y-6">
      <header class="space-y-3 border-b border-zinc-800 pb-6">
        ...
        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">{{ ui.progress }}</p>
