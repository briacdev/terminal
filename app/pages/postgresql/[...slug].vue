...
import type { PostgresqlRoadmapPost } from '../../composables/usePostgresqlRoadmaps'
import { normalizePostgresqlRoadmapPath } from '../../data/postgresql-roadmap'
...
const path = computed(() => `/postgresql/${slug.value.join('/')}`)
const legacyEnPath = computed(() => `/roadmaps/postgresql/${slug.value.join('/')}`)
const legacyFrPath = computed(() => `/roadmaps-fr/postgresql/${slug.value.join('/')}`)

const queryCollectionLoose = queryCollection as unknown as (collection: string) => {
  path: (value: string) => { first: () => Promise<PostgresqlRoadmapPost | null> }
}

let post: PostgresqlRoadmapPost | null = null
...
if (!post) {
  post = (await queryCollection('roadmaps').path(path.value).first()) as PostgresqlRoadmapPost | null
  if (!post) {
    post = (await queryCollection('roadmaps').path(legacyEnPath.value).first()) as PostgresqlRoadmapPost | null
  }
}

if (!post) {
  throw createError({ statusCode: 404, statusMessage: 'PostgreSQL roadmap article not found' })
}

const allPosts = await usePostgresqlRoadmaps()
const currentIndex = computed(() =>
  allPosts.findIndex((item: PostgresqlRoadmapPost) => item.path === normalizePostgresqlRoadmapPath(post.path))
)
...
const ui = computed(() =>
  locale.value === 'fr'
    ? {
        back: 'Retour à la roadmap PostgreSQL',
        previous: 'Étape précédente',
        next: 'Étape suivante',
        toc: 'Table des matières',
        noHeadings: 'Aucun titre dans cette page.',
        navAria: 'Navigation des pages de la roadmap PostgreSQL'
      }
    : {
        back: 'Back to PostgreSQL Roadmap',
        previous: 'Previous step',
        next: 'Next step',
        toc: 'Table of contents',
        noHeadings: 'No headings in this page.',
        navAria: 'PostgreSQL roadmap article navigation'
      }
)
